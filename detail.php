...
<section id="nleft">
 ...
 try {
    ...
    $stmt = $pdo->query("SELECT task.`task-id`,task.title, member_details.photo, task.priority, task.status,task.startdate,task.enddate 
    FROM task JOIN member_details  ON task.`ass-to` = member_details.id  and 
...
if (isset($_POST['submit'])) {
  try {
    ...
    $sql = "UPDATE task SET status = :status WHERE `task-id` = :taskId";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':status', $_POST['status']);
    $stmt->bindParam(':taskId', $_POST['task_id']);
    $stmt->execute();
    header("Refresh:0");
     echo $row['status']; 
...
<section id="tab">
      ...
      <tr>
        ...
        <td>  <form  method="post">
      <input type="hidden" name="task-id" value="<?php echo $row['task-id']; ?>">
  <select name="status">
    <option value="finish" <?php echo ($row['status'] == 'finish') ? 'selected' : ''; ?>>Finish</option>
    <option value="active" <?php echo ($row['status'] == 'active') ? 'selected' : ''; ?>>active</option>
    <option value="active" <?php echo ($row['pending'] == 'pending') ? 'selected' : ''; ?>>pending</option>
    <option value="late" <?php echo ($row['late'] == 'late') ? 'selected' : ''; ?>>late</option>
