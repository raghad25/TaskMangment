...
try {
    ...
    $stmt = $pdo->prepare("SELECT `task-id`, enddate, status,title FROM task");
    $stmt->execute();
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    foreach($results as $task) {
      if (strtotime($task['enddate']) < time() && $task['status'] != "finish") {
        $sql = "UPDATE task SET status = 'Late' WHERE `task-id` = :taskId";
        $updateStmt = $pdo->prepare($sql);
        $updateStmt->bindParam(':taskId', $task['task-id']);
        $updateStmt->execute();
      }
    }
